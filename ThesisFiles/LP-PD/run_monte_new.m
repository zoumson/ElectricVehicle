...
trans_price = 223.6;
contract_capacity_overall = [linspace(100, 1400, 50) ;  
                             linspace(600, 2800, 50) ;
                             linspace(1500, 3500, 50)];
...
fold = 50;
monte(fold) = struct();
% monte = zeros(fold, numel(fleet_size));
...
%         for fleet_size_num = 1:1%  for-3     
        ...
        satisfied_name = strcat("Satisfied_EV_", num2str(fleet_size(fleet_size_num)));
        trans_name = strcat("PL_EV_", num2str(fleet_size(fleet_size_num)));
        utility_ev_name = strcat("Cost_EV_", num2str(fleet_size(fleet_size_num)));
        utility_agg_name = strcat("Cost_Agg_", num2str(fleet_size(fleet_size_num)));
        utility_all_name = strcat("Utility_ALL_", num2str(fleet_size(fleet_size_num)));
        contract_capacity_ALL_name = strcat("contract_capacity_ALL_", num2str(fleet_size(fleet_size_num)));
        monte(monte_generated).(trans_name) = contract_capacity_given;
        monte(monte_generated).(utility_ev_name) = utility_ev_given;
        monte(monte_generated).(utility_agg_name) = utility_trans_given;
        monte(monte_generated).(utility_all_name) = utility_given;
        monte(monte_generated).(contract_capacity_ALL_name) = contract_capacity_overall(fleet_size_num, :);   
        monte(monte_generated).(satisfied_name) = satisfied_given;   
        
% %         monte(monte_generated, fleet_size_num) = contract_capacity_given;
        delete(filename_ev(fleet_size_num));
    end % end of for-3
end % end of for-1
toc
% writematrix(monte,'contract_capacity_1.xls');
